...
function CastPage ({ Cast }) {
  ...
  async function callSpecificCast () {
    let response = await fetch(
      `https://api.themoviedb.org/3/person/${Cast.id}?api_key=${import.meta.env.VITE_apiKey}`
    )
    response = await response.json()
    setCurrentCast(response)
    response = await fetch(
      `https://api.themoviedb.org/3/person/${Cast.id}/movie_credits?api_key=${import.meta.env.VITE_apiKey}&language=en-US`
    )
    response = await response.json()
    setCombined_list(prev => [...prev, ...response.cast, ...response.crew])
    setCurrentCastMovies([...response.cast, ...response.crew])
    response = await fetch(
      `https://api.themoviedb.org/3/person/${Cast.id}/tv_credits?api_key=${import.meta.env.VITE_apiKey}&language=en-US`
    )
    response = await response.json()
